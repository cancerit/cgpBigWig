...
void setup_options(int argc, char *argv[]){
   ...
   while((iarg = getopt_long(argc, argv, "F:f:i:o:c:r:ahv",long_opts, &index)) != -1){
    switch(iarg){
      case 'F':
...
// callback for bam_plbuf_init()
static int perbase_pileup_func_overlap(uint32_t tid, uint32_t position, int n, const bam_pileup1_t *pl, void *data, uint32_t reg_start){
  tmpstruct_t *tmp = (tmpstruct_t*)data;
  int pos              = (int)position;
  int coverage         = n;
  int base_coverage    = 0;
...
int main(int argc, char *argv[]){
  ...
  int i=0;
    ...
    for(k=0;k<4;k++){
	  ...
	  chck = process_bam_region_bases(input_file, func_reg, perbase, filter, filterinc, our_region_list[i], reference);
